...
Добавьте mongodb и mongoose в package.json проекта. Тогда потребуется mongoose. Храните URI базы данных MongoDB Atlas в приватном файле `.env`  как MONGO_URI. Окружите URI одинарными или двойными кавычками и убедитесь, что между переменной и `=`, а также значением и `=` не существует пробела. Подключитесь к базе данных, используя следующий синтаксис:
