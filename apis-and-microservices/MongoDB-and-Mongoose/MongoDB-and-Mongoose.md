...
В последних версиях mongoose есть методы, упрощающие обновление документов. Некоторые более продвинутые функции (например, хуки pre/post, валидация) ведут себя по-другому при этом подходе, поэтому классический метод все еще полезен во многих ситуациях. Метод `findByIdAndUpdate()` можно использовать при поиске по `Id`.

Найдите человека по имени и установите его возраст в 20 лет. Используйте параметр функции personName в качестве ключа поиска.

Примечание: Вы должны вернуть обновленный документ. Для этого вам нужно передать документ options { new: true } в качестве 3-го аргумента функции findOneAndUpdate(). По умолчанию эти методы возвращают неизмененный объект.
