...
## Обслуживайте JSON на определенном маршруте

В то время как сервер HTML служит (вы догадались!) HTML, API обслуживает данные. API REST (REpresentational State Transfer) позволяет обмениваться данными простым способом, без необходимости для клиентов знать какие-либо детали о сервере. Клиенту нужно только знать, где находится ресурс (URL-адрес), и действие, которое он хочет выполнить на нем (глагол). Команда GET используется, когда вы получаете некоторую информацию, ничего не изменяя. В наши дни предпочтительным форматом данных для перемещения информации по сети является JSON. Проще говоря, JSON-это удобный способ представления объекта JavaScript в виде строки, поэтому он может быть легко передан.

Давайте создадим простой API, создав маршрут, который отвечает JSON на пути / json. Вы можете сделать это, как обычно, с помощью приложения.метод get. Внутри обработчика маршрута используйте метод res. json (), передавая объект в качестве аргумента. Этот метод закрывает цикл запрос-ответ, возвращая данные. За кулисами он преобразует допустимый объект JavaScript в строку, затем устанавливает соответствующие заголовки, чтобы сообщить вашему браузеру, что вы обслуживаете JSON, и отправляет данные обратно. Допустимый объект имеет обычную структуру {key: data}. данные могут быть числом, строкой, вложенным объектом или массивом. данные также могут быть переменной или результатом вызова функции, в этом случае они будут вычислены перед преобразованием в строку.

Используйте объект {"message":" Hello json"} в качестве ответа в формате JSON для получения запросов к маршруту /json. Затем наведите браузер на ваш-app-url/json, вы должны увидеть сообщение на экране.
